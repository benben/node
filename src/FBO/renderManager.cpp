...
//---------------------------------------------------------------------------
void renderManager::setLeftWarp(int whichScreen, float centerPointPct, float startWarp, float endWarp){
    if( nScreens <= 0 ) return;
    if( whichScreen >= nScreens ) return;

    for(int y = 1; y < GRID_Y-1; y++){
        float ypct = (float)y/(GRID_Y-1);
        float dist = ypct - centerPointPct;

        compL[whichScreen][y] = startWarp + dist * endWarp;
    }
}

//---------------------------------------------------------------------------
void renderManager::setRightWarp(int whichScreen, float centerPointPct, float startWarp, float endWarp){
    if( nScreens <= 0 ) return;
    if( whichScreen >= nScreens ) return;
    ...
    for(int y = 1; y < GRID_Y-1; y++){
        ...
        compR[whichScreen][y] = startWarp + dist * endWarp;
    }
}
...
void renderManager::drawInputDiagnostically(float x, float y, float w, float h){
   if( nScreens <= 0 ) return;
	...
	ofNoFill();
		...
		float drawSWidth = w / nScreens;
		for(int i = 0; i < nScreens; i++){
			ofRect(x + drawSWidth * i, y, drawSWidth, h);
		}
    ofFill();
...
void renderManager::drawOutputDiagnostically(float x, float y, float w, float h){
    ...
    ofNoFill();
		...
		float drawSWidth = w / nScreens;
		for(int i = 0; i < nScreens; i++){
			ofRect(drawSWidth * i, 0, drawSWidth, h);
		}
    ofFill();
